...
def main():
    ...
    FileLength = com_fo_nlp.Nsum1-2*mpc_horizon
    RLfoot_com_pos = np.zeros([9, com_fo_nlp.Nsum1-2*mpc_horizon])
    t_long = np.arange(0,(com_fo_nlp.Nsum1-2*mpc_horizon)*dt,dt)    
    ...
    while i < FileLength:
        ...
        if t < t_homing:  ############# initial pose
            ...
            if Freebase: #routine1: change the base position and orientation for pinocchio IK: time-cost process due to the redundant freedom
                ...
                ##------ kMP based swing leg trajectory -----######### 
                rfoot_p, lfoot_p, landing_reference = com_fo_nlp.Bezier_foot_trajectory(j, dt_sample, j_index, lift_height,mpc_horizon)
                ...
                for ix in range(mpc_horizon):
                    X_ref[0,ix] = copy.deepcopy((-com_intex[0, ix]- 0 + base_home_fix[0]))
                    X_ref[1,ix] = copy.deepcopy((com_intex[1, ix]- 0 + base_home_fix[1]))
                    X_ref[2,ix] = copy.deepcopy((com_intex[2, ix]- hcomx + base_home_fix[2]))
                ...
                if(open_loop_mpc):
                    ...
                    x_init = np.hstack((com_feedback_det,angle_feedback_det,comv_feedback_det,angular_v_feedback_det))                 
                # if(j%20==0):
                #     print("x_init:",x_init)

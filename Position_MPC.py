...
def generate_optimal_controller():
    ...
    J_ini = cp.Parameter((m_inertial,m_inertial),"Inertial_matrix")
    J_ini.value = np.zeros((m_inertial,m_inertial))
    land_ref = cp.Parameter((n_cmp,N),name="land_ref")
    land_ref = np.zeros((n_cmp,N))
    ...
    for i in range(N):
        ...
        constr += [X_cmp[0,i+1] == X[0,i+1] - U[0,i]/g * Z_c[0] - J_ini[1,1]*U[4,i]/(mass*g)] ### cmpx computing
        constr += [X_cmp[1,i+1] == X[1,i+1] - U[1,i]/g * Z_c[0] + J_ini[0,0]*U[3,i]/(mass*g)] ### cmpy computing
    ...
    Q = 1*np.array([300,300,300,50,50,50,90,30,30,10,10,10])*np.eye(n)
    Q_cmp = 1*np.array([10,10])*np.eye(n_cmp)
    cost = cp.sum_squares(Q@(X[:,1:N+1]-X_ref))+ cp.sum_squares(R@U) + cp.sum_squares(Q_cmp@(X_cmp[:,1:N+1]-land_ref))
    ...
    cpg.generate_code(prob,code_dir='mpc_code_gen',solver='ECOS')#,solver='ECOS')
